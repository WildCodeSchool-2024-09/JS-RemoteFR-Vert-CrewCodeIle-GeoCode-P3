...
export default function AdminUsersPage() {
  ...
  const handleChangeActualUser = async (userData: AdminUserProps) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/admin/user/${actualUser?.id}`,
        {
          method: "PUT",
      ...
      if (response.ok) {
        ...
        const updateUserList = usersList.map((e) => {
          if (updatedUserData.id === e.id) {
            return { ...e, ...userData };
          }
          return e;
        });
  ...
  const handleDeleteUser = (id: number) => {
    fetch(`${import.meta.env.VITE_API_URL}/api/admin/user/${id}`, {
      ...
      if (res.ok) {
        setUsersList((prevState) => prevState.filter((e) => e.id !== id));
        setActualUser(null);
  ...
  return (
    <>
      ...
      <ConfirmDelete
        handleDeleteMessage={handleDeleteUser}
        isConfirmDeleteModale={isConfirmDeleteModale}
        setIsConfirmDeleteModale={setIsConfirmDeleteModale}
        actualMessage={actualUser && actualUser}
      />
    </>
  );
}
